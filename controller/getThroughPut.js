...
module.exports=function(req,res){
	...
	if(startTime&&endTime&&interval)
	{
		...
		{
			Q.all([ throughPut(timeObj,'RX'), throughPut(timeObj,'TX') ]).then(function(responseData){
				
					Q.all([sumUpThroughPut(responseData[0],'RX',interval),sumUpThroughPut(responseData[1],'TX',interval)]).then(function(sumupData){
...
function throughPut(timeStamp,type){
    var deferred = Q.defer();
	db.throughPutInfo.find({ timeStamp : { $gte : timeStamp.start, $lte : timeStamp.end} ,Type:type}).toArray(function(err,data){
		//console.log(data);
		if(err)
		{
			deferred.reject(err);
...
function sumUpThroughPut(array,type,interval){
		var deferred = Q.defer();
	    if(interval=="hour")
	    {
	    	var throughPut=0;
			for(var i=0;i<array.length;i++)
			{
					if(type=="RX")
					{
						  throughPut=throughPut+array[i].ThroughtPutRXMax;

					}else if(type=="TX"){
 						
						 throughPut=throughPut+array[i].ThroughtPutTXMax;
					}
			}
			deferred.resolve([{"throughPutData":throughPut}]);
	    	...
	    	for(var t=0;t<collectionOfData.length;t++)
	    	{
	    		...
	    		for(var k=0;k<collectionOfData[t].length;k++)
	    		{
	    			...
	    			if(type=="RX")
					{
 						throughPut=throughPut+volueDataContainer[k].ThroughtPutRXMax;
	 				}else if(type=="TX"){
						throughPut=throughPut+volueDataContainer[k].ThroughtPutTXMax;
	 				}
		   		}
	    		responseDataVolume.push({"throughPutData":throughPut})
	      	}
	    	...
	    	for(var t=0;t<collectionOfData.length;t++)
	    	{
	    		...
	    		for(var k=0;k<collectionOfData[t].length;k++)
	    		{
	    			var throughPutContainer=collectionOfData[t];
	    			if(type=="RX")
					{
						throughPut=throughPut+throughPutContainer[k].ThroughtPutRXMax;

	 				}else if(type=="TX")
	 				{
	 					throughPut=throughPut+throughPutContainer[k].ThroughtPutTXMax;
	 				}
	    		}
	    		responseDataVolume.push({"throughPutData":throughPut})
	      	}
